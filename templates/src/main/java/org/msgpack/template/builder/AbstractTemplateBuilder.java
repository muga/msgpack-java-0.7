...
public abstract class AbstractTemplateBuilder implements TemplateBuilder {
	...
	@Override
	public <T> Template<T> buildPackerTemplate(final Type targetType)
		...
		return buildPackerTemplate(targetClass, entries);
	}

	@Override
	public <T> Template<T> buildPackerTemplate(final Class<T> targetClass,
	...
	protected abstract <T> Template<T> buildPackerTemplate(
			Class<T> targetClass, FieldEntry[] entries);

	@Override
	public <T> Template<T> buildUnpackerTemplate(final Type targetType)
			throws TemplateBuildException {
		@SuppressWarnings("unchecked")
		Class<T> targetClass = (Class<T>) targetType;
		checkClassValidation(targetClass);
		FieldOption fieldOption = getFieldOption(targetClass);
		FieldEntry[] entries = toFieldEntries(targetClass, fieldOption);
		return buildUnpackerTemplate(targetClass, entries);
	}

	@Override
	public <T> Template<T> buildUnpackerTemplate(final Class<T> targetClass,
			final FieldList fieldList) throws TemplateBuildException {
		checkClassValidation(targetClass);
		FieldEntry[] entries = toFieldEntries(targetClass, fieldList);
		return buildUnpackerTemplate(targetClass, entries);
	...
	protected abstract <T> Template<T> buildUnpackerTemplate(Class<T> targetClass, FieldEntry[] entries);

    protected void checkClassValidation(final Class<?> targetClass) { // TODO
